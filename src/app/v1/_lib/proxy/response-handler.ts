...
// semantics). For these, subtract both cache buckets from input_tokens. A
// per-provider setting extends cache-read subtraction to compatible upstreams
// without changing their cache-write semantics.
...
function adjustUsageForProviderType(
  ...
  const providerUsesCacheSubset =
    providerType !== null &&
    providerType !== undefined &&
    PROVIDERS_WITH_CACHE_SUBSET_USAGE.has(providerType);
  if (!providerUsesCacheSubset && !inputTokensIncludeCacheRead) {
    return usage;
  }
  ...
  const cacheWriteTokens =
    providerUsesCacheSubset && rawUsage ? (getNestedOpenAICacheWriteTokens(rawUsage) ?? 0) : 0;
